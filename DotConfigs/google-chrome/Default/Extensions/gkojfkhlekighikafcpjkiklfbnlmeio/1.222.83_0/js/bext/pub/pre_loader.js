...
const hola = self.hola = self.hola||{};
const E = hola.base = {};
function _zerr(){
    if (typeof self!='object' || !self.console || !self.console.error)
        return;
    Function.prototype.apply.bind(self.console.error,
...
const qs = (param, opt)=>{
    ...
    if (!param)
        return uri;
    opt = opt||{};
    var _uri_comp = opt.space_plus ? uri_comp : self.encodeURIComponent;
    for (var i in param)
    {
        var val = param[i];
...
E.get_conf = ()=>self.conf||E.defaults.conf;
E.get_zon_config = ()=>self.zon_config||E.defaults.zon_config;
...
E.raw_perr = opt=>{
    try {
        ...
        fetch(url+'?'+qs(opt, {space_plus: true}), {method: 'POST'});
    } catch(e){ _zerr('raw_perr failed '+e.stack); }
};

var get_hola_extension_info = ()=>{
    if (self.hola_extension_info)
...
E.perr_prepare = opt=>{
    ...
    opt.cid = opt.cid || self.hola_svc_info&&self.hola_svc_info.cid;
...
E.build_info = opt=>{
    ...
    info.browser_build = self.conf && self.conf.browser.name ||
        E.browser();
...
var prev_error_handler = self.onerror;
self.onerror = E.onerror = function(message, file, line, col, err){
    ...
    if (prev_error_handler)
        prev_error_handler.apply(self, arguments);
};

if (performance.mark)
    performance.mark('pre_loader');

if (self.localStorage && self.localStorage.getItem('rebranding')>0)
    document.documentElement.classList.add('rebranding');
})();
